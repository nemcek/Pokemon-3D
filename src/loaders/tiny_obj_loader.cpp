...
std::string LoadObj(std::vector<shape_t> &shapes,
                    ...
                    const char *filename, center_point_t *center_point, const char *mtl_basepath) {
  ...
  return LoadObj(shapes, materials, ifs, matFileReader, center_point);
}

std::string LoadObj(std::vector<shape_t> &shapes,
                    std::vector<material_t> &materials, // [output]
                    std::istream &inStream, MaterialReader &readMatFn, center_point_t *center_point) {
  std::stringstream err;
  ...
  while (inStream.peek() != -1) {
    ...
    if (token[0] == 'v' && isSpace((token[1]))) {
      ...
      if (x > center_point->X_max)
        center_point->X_max = x;
      if (x < center_point->X_min)
        center_point->X_min = x;
      if (y > center_point->Y_max)
        center_point->Y_max = y;
      if (y < center_point->Y_min)
        center_point->Y_min = y;
      if (z > center_point->Z_max)
        center_point->Z_max = z;
      if (z < center_point->Z_min)
        center_point->Z_min = z;
