...
void Mesh::initGeometry(const std::string &obj_file) {
    ...
    this->texturedModel->rawModel = this->loader->load(vertex_buffer, texcoord_buffer, index_data, normals_data);
...
//    glGenVertexArrays(1, &this->texturedModel->rawModel->vao);
//    glBindVertexArray(this->texturedModel->rawModel->vao);
//
//    // --- Vertex positions ---
//    std::vector<GLfloat> vertex_buffer;
//    for (int i = 0; i < (int) mesh.positions.size() / 3; i++) {
...
//    glGenBuffers(1, &this->texturedModel->rawModel->vbo);
//    glBindBuffer(GL_ARRAY_BUFFER, this->texturedModel->rawModel->vbo);
...
//    glGenBuffers(1, &this->texturedModel->rawModel->tbo);
//    glBindBuffer(GL_ARRAY_BUFFER, this->texturedModel->rawModel->tbo);
...
//    glGenBuffers(1, &this->texturedModel->rawModel->ibo);
//    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, this->texturedModel->rawModel->ibo);
...
//    }
//
//    glGenBuffers(1, &this->texturedModel->rawModel->nbo);
//    glBindBuffer(GL_ARRAY_BUFFER, this->texturedModel->rawModel->nbo);
