 ...
 */
void Arcball::mouseButtonCallback( GLFWwindow * window, int button, int action, int ){
...
void Arcball::cursorCallback( GLFWwindow *, double x, double y ){
    if( mouseEvent == 0 ) {
        return;
    }
    else if( mouseEvent == 1 && first_move ) {
...
glm::mat4 Arcball::createViewRotationMatrix(glm::mat4 mat) {
    return glm::rotate( mat, glm::degrees(angle) * rollSpeed, camAxis );
}
